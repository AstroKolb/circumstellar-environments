...
INTEGER :: i, j, imn, ipn, kmn, kpn, jmn, jpn, kc, jc, image1_pe, image2_pe, count1, count2, mpitag1, mpitag2
...
REAL, DIMENSION(imax,kmax) :: dens1, dens0
...
!======================================================================
!if (mype==0) print *, 'Starting images'
...
if (jcol == npey/2) then   ! pull together an r-phi (at theta=pi/2) slice on the yang grid
  ...
  enddo
  count1 = imax*ks
  call MPI_GATHER(buffone,count1,VH1_DATATYPE,dens0,count1,VH1_DATATYPE,npez/2,MPI_COMM_COL, mpierr)

  if (krow==npez/2) then

    ! root sends this data over to image2_pe collecting yin grid data
    if (yin) then
     call MPI_SEND(dens0, count, VH1_DATATYPE, image1_pe, mpitag1, MPI_COMM_WORLD, mpierr)
    else
     call MPI_RECV(dens1, count, VH1_DATATYPE, image2_pe, mpitag1, MPI_COMM_WORLD, yystat, mpierr)
    endif

    ! repeat operation, but in yang/yin order
    if (yin) then 
     call MPI_RECV(dens1, count, VH1_DATATYPE, image1_pe, mpitag2, MPI_COMM_WORLD, yystat, mpierr)
    else
     call MPI_SEND(dens0, count, VH1_DATATYPE, image2_pe, mpitag2, MPI_COMM_WORLD, mpierr)
    endif
...
if ((mype==image1_pe).or.(mype==image2_pe)) then     ! only this unique processor has both data slices
