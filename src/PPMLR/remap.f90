...
! LOCALS
INTEGER :: n, nn
REAL, DIMENSION(maxsweep) :: du, dv, dw, de, dq, dc, dr
REAL, DIMENSION(maxsweep) :: ul, vl, wl, el, ql, cl, rl
REAL, DIMENSION(maxsweep) :: u6, v6, w6, e6, q6, c6, r6
REAL, DIMENSION(maxsweep) :: dm, dm0, delta, dvol0
REAL, DIMENSION(maxsweep) :: fluxr, fluxu, fluxv, fluxw, fluxe, fluxq, fluxc
REAL :: fractn, fractn2, ekin, deltx, rad

REAL, PARAMETER :: third  = 1.0d0 / 3.0d0
REAL, PARAMETER :: twothd = 2.0d0 / 3.0d0
REAL, PARAMETER :: fourthd= 4.0d0 / 3.0d0
...
call parabola(nmin-1, nmax+1, para, c, dc, c6, cl, flat)
...
fluxr = 0.0d0  ! zero out fluxes to ensure nmin-1 and nmax+2 are zero
fluxu = 0.0d0
fluxv = 0.0d0
fluxw = 0.0d0
fluxe = 0.0d0
...
do n = nmin, nmax + 1
  ...
  if(deltx >= 0.0d0) then
    nn = n - 1
    fractn  = 0.5d0*deltx/dx(nn)
    fractn2 = 1.0d0 - fourthd*fractn
    ...
    fluxc (n) = (cl(nn) + dc(nn) - fractn*(dc(nn) - fractn2*c6(nn)))*fluxr(n)
  else
    ...
    fluxc(n) = (cl(n) - fractn*(dc(n) + fractn2*c6(n)))*fluxr(n)
  endif
enddo
...
do n = nmin-1, nmax+1  ! must update nmin-1, nmax+1 for possible second remap
  ...
  c  (n) = (c(n)*dm(n) + fluxc(n)-fluxc(n+1))*dm0(n)
enddo
         
! If flow is highly supersonic use remap on internal energy, else on total E

do n = nmin-1, nmax+1
  ekin = 0.5d0*(u(n)**2+v(n)**2+w(n)**2)
  if(ekin/q(n) < 20.0) q(n) = e(n) - ekin
  p(n) = gamm*r(n)*q(n)
  p(n) = max(smallp,p(n))
enddo
